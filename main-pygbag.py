...
async def main():
    ...
    def getdata():
        ...
        while True:
            ...
            try:
                wxdata = r.get(
                    f"https://wx.lewolfyt.cc/?loc={loc}" + ("" if not metric else "&units=m") + "&extendeddays=10"
                ).json()
            ...
            if "al" in flavor:
                ...
                if wxdata:
                    lat, long = wxdata["current"]["info"]["geocode"]
                    sr1 = r.get(f"https://api.sunrisesunset.io/json?lat={lat}&lng={long}&time_format=unix").json()["results"]
                    sr2 = r.get(f"https://api.sunrisesunset.io/json?lat={lat}&lng={long}&time_format=unix&date=tomorrow").json()[
                        "results"
                    ]
                    aldata["sun"] = {
                        "sunrise1": int(sr1["sunrise"]),
                ...
                try:
                    if radar_provider == "apollo":
                        radardt = pg.image.load_animation(
                            BytesIO(r.get(f"http://apollo.us.com:8008/radar_composite_animate.gif").content)
                        )
                        radardata2 = []
                        lat, long = wxdata["current"]["info"]["geocode"]
    ...
    if musicpath:
        for file in os.listdir(musicpath):
            if file.endswith((".ogg", ".ogg", ".flac", ".ogg", ".mod", ".ogg")) and not file.startswith("."):
    ...
    def drawshadow(
        ...
        if jr:
            ...
            if bs:
                ...
                for i, char in enumerate(text):
                    drawchar(
                        char, sheet[0], x + xx + 2 + char_offsets.get(char, 0), y + 2, color if type(color) != list else color[i]
                    )
                    if variable:
                        xx += (variable[chars.index(char)] + 1) if variable[chars.index(char)] else 15
                    else:
