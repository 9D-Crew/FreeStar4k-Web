...
def wx_url(params=None):
    return make_url("https://wx.lewolfyt.cc/", params)


def apollo_radar_url(filename):
    direct = f"http://apollo.us.com:8008/{filename}"
    if not IS_WEB:
        return direct
    cache_bust = int(tm.time() // 300)
    proxied = f"apollo.us.com:8008/{filename}?v={cache_bust}"
    return make_url("https://images.weserv.nl/", {"url": proxied})


async def _desktop_get(url):
    if r is None:
        raise RuntimeError("requests is not available")
    if hasattr(asyncio, "to_thread"):
        return await asyncio.to_thread(r.get, url)
    return r.get(url)


def _browser_fetch_api():
    try:
    ...
    try:
        ...
        if window is not None:
            return window.fetch, None
    except Exception:
        pass

    raise RuntimeError("No browser fetch API is available")


async def _browser_fetch_text_with_jsiter(url):
    global FREESTAR_FETCH_JS_READY
    import platform as web_platform

    window = getattr(web_platform, "window", None)
    jsiter = getattr(web_platform, "jsiter", None)
    if window is None or jsiter is None:
...
window.FreeStarFetch.GET = function * GET(url)
{
    var done = false;
    var content = "";
    fetch(new Request(url, {method: "GET", cache: "no-store", credentials: "omit"}))
        .then(function(resp) {
            if (!resp.ok) {
        ...
        .catch(function(err) {
            content = "__FREESTAR_FETCH_ERROR__" + String(err);
            done = true;
        });
    while (!done) {
        yield;
...
window.FreeStarFetch.BYTES = function * BYTES(url)
{
    var done = false;
    var content = "";
    fetch(new Request(url, {method: "GET", cache: "no-store", credentials: "omit"}))
    ...
    if content.startswith("__FREESTAR_FETCH_ERROR__"):
        raise RuntimeError(content.replace("__FREESTAR_FETCH_ERROR__", "", 1))
    return content


async def _browser_fetch_bytes_with_jsiter(url):
    import platform as web_platform
    ...
    if not FREESTAR_FETCH_JS_READY:
        await _browser_fetch_text_with_jsiter("data:text/plain,")

    content = await jsiter(window.FreeStarFetch.BYTES(url))
    if content.startswith("__FREESTAR_FETCH_ERROR__"):
        raise RuntimeError(content.replace("__FREESTAR_FETCH_ERROR__", "", 1))
...
async def _browser_fetch_text(url):
    try:
        return await _browser_fetch_text_with_jsiter(url)
    except Exception as jsiter_exc:
        print(f"js fetch failed for {url}: {jsiter_exc}")
    ...
    try:
        ...
        print(f"aio.fetch failed for {url}: {aio_exc}")

    fetch, js_mod = _browser_fetch_api()
    response = await fetch(url)
    status = int(getattr(response, "status", 200))
    ok = bool(getattr(response, "ok", status < 400))
    if not ok or status >= 400:
        raise RuntimeError(f"Fetch failed with HTTP {status}: {url}")
    return await response.text()


async def _browser_fetch_bytes(url):
    try:
        return await _browser_fetch_bytes_with_jsiter(url)
    except Exception as jsiter_exc:
        print(f"js binary fetch failed for {url}: {jsiter_exc}")
    ...
    if js_mod is not None:
        return bytes(js_mod.Uint8Array.new(array_buffer).to_py())
    if hasattr(array_buffer, "to_py"):
        return bytes(array_buffer.to_py())
...
async def http_get_text(url):
    ...
    response = await _desktop_get(url)
    return response.text


async def http_get_json(url):
    if IS_WEB:
        return json.loads(await _browser_fetch_text(url))
    response = await _desktop_get(url)
    return response.json()


async def http_get_bytes(url):
    if IS_WEB:
        return await _browser_fetch_bytes(url)
    response = await _desktop_get(url)
...
async def getdata_async():
    ...
    while True:
        ...
        sendtoall(f"led 6 1\n".encode())
        try:
            wxdata = await http_get_json(wx_url({"loc": loc, **({"units": "m"} if metric else {}), "extendeddays": 10}))
            primary_weather_loaded = True
            ...
            if wxdata["current"]["alerts"]:
                ...
                almap = {}
                if keylist:
                    ll = make_url("https://wx.lewolfyt.cc/alerts", {"alertkey": keylist})
                    print(ll)
                    details = (await http_get_json(ll))["alerts"]
        ...
        if "ti" in flavor:
            ...
            t1 = aldata["tidal"][0]
            print(wx_url({"id": tidal[0]}))
            t1data = (await http_get_json(make_url("https://wx.lewolfyt.cc/tides", {"id": tidal[0]})))["tides"]
            t1list = {"lows": [], "highs": []}
            for i in range(4):
                fts = dt.datetime.fromtimestamp(t1data[i]["valid"])
                t1list[["lows", "highs"][t1data[i]["type"] == "H"]].append(
                    (splubby_the_return(fts.strftime("%I:%M%p").lower() + " " + fts.strftime("%a")), t1data[i]["valid"])
                )
            aldata["tidal"][0] = t1list

            t2 = aldata["tidal"][1]
            t2data = (await http_get_json(make_url("https://wx.lewolfyt.cc/tides", {"id": tidal[1]})))["tides"]
            t2list = {"lows": [], "highs": []}
            for i in range(4):
        ...
        async def get_obsloc(l):
            global leds
            try:
                l[2] = await http_get_json(wx_url({"loc": l[0], "include": "current", **({"units": "m"} if metric else {})}))
                leds[1] = True
            except:
                print(tb.format_exc())
        ...
        async def get_tcfloc(l):
            global leds
            try:
                l[2] = await http_get_json(wx_url({"loc": l[0], "include": "extended", **({"units": "m"} if metric else {})}))
        ...
        if "lo" in flavor:
            await asyncio.gather(*(get_obsloc(l) for l in obsloc))
        if "ro" in flavor:
            await asyncio.gather(*(get_regloc(rg) for rg in reglocs))
        if "tcf" in flavor:
            await asyncio.gather(*(get_tcfloc(tc) for tc in tcflocs))

        await asyncio.sleep(0)

        try:
            report = await http_get_text(
        ...
        if "lr" in flavor:
            try:
                if radar_provider == "apollo":
                    radardt = pg.image.load_animation(
                        BytesIO(await http_get_bytes(apollo_radar_url("radar_composite_animate.gif")))
                    )
                    radardata2 = []
                    lat, long = wxdata["current"]["info"]["geocode"]
            ...
            try:
                if radar_provider == "apollo":
                    radardt = pg.image.load(BytesIO(await http_get_bytes(apollo_radar_url("radar_composite.png"))))
                    radardata2 = []
                    lat, long = wxdata["current"]["info"]["geocode"]
                    x, y = mapper((mappoint1, mappoint2), lat, long)
                    x = max(x, 0)
        ...
        sendtoall(f"led 6 0\n".encode())
        gc.collect()
        retry_delay = 300 if primary_weather_loaded else 15
...
weather_task = None
weather_task_started = False


def _weather_task_done(task):
    try:
        task.result()
...
def drawchar(char, cset, x, y, color, half=0):
    ...
    win.blit(
        cset2,
        (x, y),
        pg.Rect((ix * 32) % cset.get_width(), int(ix * 32 // cset.get_width()) * (cset.get_height() // 6), 32, cheight),
    )
    ...
    if mono == 0:
        ...
        for i, char in enumerate(text):
            ...
            if font in font_tallest:
                coffset2 = font_tallest[font] - font.size(char)[1]
            surface.blit(
                scalec(frender(font, char, True, col), font, char, col, (fw if ofw is None else ofw, 1)),
...
def drawpage(lines: list, smalltext="", shift=0, vshift=0):
    ...
    for i, line in enumerate(clines):
        ...
        if ld == 8:
            break
        drawshadow(
            starfont32, line, 80 + xoff, 109 + linespacing * (i - ss) + ldl_y * 1.25 + vshift, 3, mono=gmono, char_offsets={}
        )
    if smalltext:
...
if screenw > 768:
    radarLeft = pg.transform.scale(
        radarHeader.subsurface(pg.Rect(0, 0, 1, radarHeader.get_height())), (m.ceil((screenw - 768) / 2), radarHeader.get_height())
    )
    radarRight = pg.transform.scale(
        radarHeader.subsurface(pg.Rect(radarHeader.get_width() - 1, 0, 1, radarHeader.get_height())),
...
if screenw > 768:
    ...
    radarRightC = pg.transform.scale(
        radarHeaderC.subsurface(pg.Rect(radarHeaderC.get_width() - 1, 0, 1, radarHeaderC.get_height())),
        (m.ceil((screenw - 768) / 2), radarHeaderC.get_height()),
    )
...
working = True
cl = BrowserClock() if IS_WEB else AccuraterClock()
...
async def main():
    ...
    global crawl, crawlactive, crawling, crawlinterval, crawlscroll, crawltime, d, d1
    global d2, dat, date, day, delta, dr, event, ext
    global ext_action, f, fc, fcst, fcsts, fctx, finallines, fired
    global frame_idx_actual, generaldrawidx, header, hi, hour, i, iconidx, iconidx3
    global intropicked, introtx, ioff, it, ix, j, jrf, l
    global lastlasttime, lasttime, lc, ldl_y, ldl_y_off, ldldrawidx, ldlextra, ldlidx
    global ldlinterval, ldlmode, ldlon, ldlreps, ldlspace, ldltext, lin, line
    global lines, ln, lo, max_y, mm, mn, moondt, mt
    global name, nextcrawlready, nn, ol, ooo, p, p_counter, page
    global pl, post, pre, precip_type, ps, pt, radaridx, radartime
    global roll, rt, s1, s2, sanitize, serial, showing, sl
    global slide, slideinterval, spec, sq, sq2, squeeze, stat, subpage
    global sunrise1, sunrise2, sunset1, sunset2, tcl, text, textix, textixx
    global textpos, texts, time, times, to, tr, transform, tx
    global txo, veryuppercase, wc, weather_task, weather_task_started, wndtext, working, ws, wt, xi
    global xx, yo, yoo, your, yy

    if IS_WEB and not weather_task_started:
        weather_task = asyncio.create_task(getdata_async())
        weather_task.add_done_callback(_weather_task_done)
    ...
    while working:
            ...
            if slide in ["cc", "oldcc"]:
                if slide == "oldcc":
                    ...
                    if wxdata is None:
                        if veryuppercase:
                            ...
                            drawshadow(
                                starfont32, "       No Report Available", 80 + txoff, 109 + linespacing * 2.5 + ldl_y, 3, mono=gmono
                            )
                    else:
                        page = [ccphrase]
                    ...
                    if not wxdata:
                        ...
                        drawshadow(
                            starfont32, locname, 367 + txoff, 91 + ldl_y, 3, color=yeller, ofw=1.07, mono=15, upper=veryuppercase
                        )
                        ...
                        drawshadow(
                            starfont32,
                            "Pressure :" if ("ccspace" in old and not metric) else "Pressure:",
                            394 + txoff,
                            304 + ldl_y,
                            3,
                            ofw=1.07,
                        ...
                        drawshadow(
                            starfont32, bp, 537 - 12 * metric + txoff, 304 + ldl_y, 3, mono=18, char_offsets={}, upper=veryuppercase
                        )
                        pt = wxdata["current"]["conditions"]["pressureTendency"]
                        if pt == 0:
                            drawshadow(
                ...
                for l in obsloc:
                    if l[2] == None:
                        ...
                        if ws > 9:
                            ol = textmerge(
                                ol, f"                            {windreduce(l[2]['current']['conditions']['windCardinal'])}"
                            )
                ...
                for alert in alerts:
                    ...
                    for line in allines:
                        finallines.append(textmerge("*" + 30 * " " + "*", m.ceil(16 - len(line) / 2) * " " + line))
                    finallines.append("\n")
                ...
                if wxdata:
                    if not alert36:
                        ...
                        #     txt += "\n"
                        text = (
                            (fcsts[subpage]["name"][0].upper() + fcsts[subpage]["name"][1:].lower())
                            + "..."
                        ...
                        for textixx in range(4):
                            texts.append(
                                wraptext(
                                    (fcsts[textixx]["name"][0].upper() + fcsts[textixx]["name"][1:].lower())
                                    + "..."
                ...
                if aldata["moon"]:
                    ...
                    for i in range(4):
                        ...
                        xx = i * 151
                        drawshadow(
                            starfont32, mt, 112 + 18 * 1.5 - len(mt) * 9 + xx + txoff, 224 + ldl_y, 3, mono=gmono, char_offsets={}
                        )

                        dat = padtext(moondt[1], 6)
                        mn = pg.transform.smoothscale_by(
                            {"New": moon_new, "First": moon_fq, "Full": moon_full, "Last": moon_lq}[mt], (1.2, 1)
                        )
                ...
                for i, name in enumerate(tcflocs):
                    ...
                    if tcflocs[i][2]:
                        drawshadow(
                            largefont32,
                            padtext(str(tcflocs[i][2]["extended"]["daily"][1]["tempMin"]), 3),
                            540 - 21,
                            72 * i + 120 + yy + ldl_y,
                            3,
                            color=yeller,
                        ...
                        drawshadow(
                            largefont32,
                            padtext(str(tcflocs[i][2]["extended"]["daily"][1]["tempMax"]), 3),
                            613 - 21,
                            72 * i + 120 + yy + ldl_y,
                            3,
                            color=yeller,
                ...
                if aldata["tidal"][0]:
                    lines[1] = textmerge(
                        textmerge("Lows:", " " * 7 + padtext(aldata["tidal"][0]["lows"][0][0], 11)),
                        " " * 21 + padtext(aldata["tidal"][0]["lows"][1][0], 11),
                    )
                    lines[2] = textmerge(
                        textmerge("Highs:", " " * 7 + padtext(aldata["tidal"][0]["highs"][0][0], 11)),
                ...
                if aldata["tidal"][1]:
                    lines[4] = textmerge(
                        textmerge("Lows:", " " * 7 + padtext(aldata["tidal"][1]["lows"][0][0], 11)),
                        " " * 21 + padtext(aldata["tidal"][1]["lows"][1][0], 11),
                    )
                    lines[5] = textmerge(
                        textmerge("Highs:", " " * 7 + padtext(aldata["tidal"][1]["highs"][0][0], 11)),
                ...
                if aldata["sun"]:
                    ...
                    lines[6] = textmerge(
                        textmerge("   Sunrise         Set", " " * 11 + splubby_the_return(s1.strftime("%I:%M%p").lower())),
            ...
            if serial:
                ...
                if ldlidx == 0:
                    ...
                    if wxdata is not None:
                        ...
                        if wxdata["current"]["conditions"]["windGusts"] is not None:
                            ldltext = textmerge(
                                ldltext, f'{ldlspace}                  Gusts to  {wxdata["current"]["conditions"]["windGusts"]}'
                            )
                elif ldlidx == 6:
                    if wxdata is not None:
                        ceil = f':{padtext(wxdata["current"]["conditions"]["cloudCeiling"], 5)}{short_dist}'
                ...
                if alerting:
                    pg.draw.rect(
                        win,
                        (
                            ((187, 17, 0) if (slide not in ["lr", "cr"] or ("warnpalbug" not in old)) else (128, 16, 0))
        ...
        if (ldlon and not serial) or not ldlmode:
            if not ui or ((slide in ["lr", "cr"]) and not ldlmode):
                ...
                drawshadow(
                    smallfont, time.upper(), 479 + round((screenw - 768) * 2 / 3) - 2, 35, 3, mono=gmono, color=tcl, char_offsets={}
                )
                drawshadow(smallfont, date.upper(), 479 + round((screenw - 768) * 2 / 3), 55, 3, mono=gmono, char_offsets={})
            elif textpos == 1:
                drawshadow(
                    smallfont, time.upper(), 465 + round((screenw - 768) * 2 / 3) - 2, 28, 3, mono=gmono, color=tcl, char_offsets={}
                )
                drawshadow(smallfont, date.upper(), 465 + round((screenw - 768) * 2 / 3), 48, 3, mono=gmono, char_offsets={})

        for ext in ext_loaded:
            if "post_draw" in ext:
                ext_action = ext["post_draw"](
                    win,
                    {
                        "textpos": textpos,
                        "ldlmode": ldlmode,
